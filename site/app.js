...
const ICS_PATH = "calendar.ics";  // served at the site root
...
function renderEvents(eventsData, programmes) {
  const list = $("#events-list");
  list.innerHTML = "";

  const programmesById = {};
  if (programmes) for (const p of programmes) programmesById[p.id] = p;
  ...
  if (events.length === 0) {
    list.innerHTML = '<div class="empty">No upcoming events — deadlines and openings will appear here as the scraper finds them</div>';
  ...
  for (const e of events) {
    ...
    const month = monthFmt.format(e.dateObj).toUpperCase();
    ...
    row.innerHTML = `
      ...
      <div>
        ...
        <div class="event-org">${escapeHTML(programme.org || "")}</div>
      </div>
      <span class="event-tag ${e.kind}">${escapeHTML(e.kind)}</span>
    `;
...
function setupCalendarButtons() {
  // Build the absolute URL to the .ics file
  const absolute = new URL(ICS_PATH, location.href).href;
  const webcal = absolute.replace(/^https?:/, "webcal:");

  const subBtn = $("#cal-subscribe");
  if (subBtn) subBtn.href = webcal;

  const copyBtn = $("#cal-copy");
  const copySub = $("#cal-copy-sub");
  if (copyBtn) {
    copyBtn.addEventListener("click", async () => {
      try {
        await navigator.clipboard.writeText(absolute);
        if (copySub) {
          const original = copySub.textContent;
          copySub.textContent = "copied ✓";
          setTimeout(() => { copySub.textContent = original; }, 2000);
        }
      } catch (err) {
        // Fallback: prompt the user to copy manually
        prompt("Copy this URL and paste it into your calendar:", absolute);
      }
    });
  }
}
...
(async function () {
  ...
  const [programmes, status, discoveries, events] = await Promise.all([
  ...
  renderStats(status);
  renderEvents(events, programmes);
