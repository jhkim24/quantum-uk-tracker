...
- **Generates a calendar feed** at `/calendar.ics` with events for every detected deadline (1-week + on-the-day reminders) and every opening (on-the-day reminder). Subscribe once in Google Calendar, Apple Calendar, or Outlook and new events appear automatically as the scraper finds them.
- **Renders a static site** with filterable cards, status pills, last-checked timestamps, an upcoming-events list, and a discoveries section.

## Subscribing to the calendar feed

The site has a "Subscribe to Calendar" panel at the top with three options:

- **Subscribe (webcal://)** — opens in your default calendar app (works on macOS/iOS Calendar, falls back gracefully on Android)
- **Download .ics** — one-time import; doesn't auto-update afterward
- **Copy URL** — paste into Google Calendar / Outlook subscription dialog

### Google Calendar (most common)
1. Click **Copy URL** on the site
2. Open Google Calendar → left sidebar → **Other calendars** → **+** → **From URL**
3. Paste, click **Add calendar**
4. The new calendar appears in your sidebar; toggle visibility with the colour swatch

Google Calendar refreshes subscribed feeds roughly every 12 hours. If you want a deadline to land instantly, the workflow's manual trigger forces a re-scrape and re-publish; Google will pick it up on its next poll.

### Apple Calendar (macOS / iOS)
1. Click **Subscribe** on the site (`webcal://` link) — Calendar opens automatically
2. Choose how often to refresh (default is fine)

### Outlook
1. Click **Copy URL**
2. Outlook web → Calendar → **Add calendar** → **Subscribe from web** → paste
3. Outlook honours VALARM reminders only on desktop, not in the web client.

### Reminders
- **Deadlines** fire 1 week before AND on the day
- **Openings** fire on the day they become live
- VALARM components are honoured on Google Calendar, Apple Calendar, and Outlook desktop. Outlook web silently drops them.
...
│   ├── calendar.py                # iCalendar feed generator
│   └── requirements.txt
├── data/
│   ├── programmes.json            # CURATED — edit by hand
│   ├── status.json                # GENERATED — overwritten each run
│   ├── events.json                # GENERATED — canonical event list
│   ├── calendar.ics               # GENERATED — published at /calendar.ics
